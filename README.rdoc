...
server can be started from the command line.
  ...
  ruby service/manifest_service.rb
  
This will start Sinatra listening on port 3000. You can specify port number by supplying "-p"
parameter. For example:

  ruby service/manifest_service.rb -p 7070
  
One the server is running, you can execute the tests by calling rspec:

  rspec spec/manifest_service_spec.rb
